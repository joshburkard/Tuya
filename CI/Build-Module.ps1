...
if($TestsResult.FailedCount -eq 0){
    ...
    $PublicFunctions  = Get-ChildItem -Path "$CodeSourcePath\public" -Filter '*-*.ps1' | sort-object Name
    $PrivateFunctions = Get-ChildItem -Path "$CodeSourcePath\private" -Filter '*-*.ps1' | sort-object Name
    $MainPSM1Contents = @()
...
"@
                        ( $Function.Parameters."$( $parameter.name )".Attributes[1].ValidValues ) | ForEach-Object {
                            $FileContent += @"
- $( $_ )
